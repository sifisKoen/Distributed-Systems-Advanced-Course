...
class EPFD(epfdInit: Init[EPFD]) extends ComponentDefinition {
  ...
  ctrl uponEvent {
    case _: Start => {
      ...
      // alive = Set(topology); // alive = Π
      // suspected = 0; // suspected = 0
      // period = delta; // delay = Δ
      startTimer(period); // startTime(delay)
    }
  }
  ...
  timer uponEvent {
    case CheckTimeout(_) => {
      if (!alive.intersect(suspected).isEmpty) {
        ...
        /* WRITE YOUR CODE HERE */
        period = period + delta; // delay = delay + Δ
      ...
      for (p <- topology) {
        if (!alive.contains(p) && !suspected.contains(p)) {
         ...
         /* WRITE YOUR CODE HERE */
          suspected = suspected + p; // suspected = suspected U {p}
          trigger(Suspect(p) -> epfd) // trigger(vP, Suspect | p)
  ...
  pLink uponEvent {
    ...
    case PL_Deliver(src, HeartbeatReply(seq)) => {
         ...
         /* WRITE YOUR CODE HERE */
         if (seqnum == seq || suspected.contains(src)) {
            alive = alive + src;
         }
    }
  }
